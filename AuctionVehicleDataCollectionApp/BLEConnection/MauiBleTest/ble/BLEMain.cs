...
// using MauiBleTest.ble.Exceptions;

// namespace MauiBleTest.ble
// {
// 	public abstract class BLEMain
...
// 		public abstract class BLEControlBase
// 		{
...
// 			public ConnectStat IsConnect { get; private set; }
...
// 			{
// 				IsConnect = ConnectStat.DISCONNECT;
...
// 				//指定したデバイス名ならば
// 				if(device.Name == _bleconInfo.TerminalName)
// 				{
...
// 			public async Task<BLERESULT> StartConnect()
// 			{
// 				await Scan();
...
// 						_service = service;
// 						//_IsRecieve = true;
// 						IsConnect = ConnectStat.CONNECT;       //指定したサービスIDを捕まえて初めて接続扱いにする
// 																//Read();
// 						startRecieve();
...
// 				// _characteristicread = await _service.GetCharacteristicAsync(Guid.Parse(_bleconInfo.ReadCharacteristic));
// 				// _characteristicwrite = await _service.GetCharacteristicAsync(Guid.Parse(_bleconInfo.WriteCharacteristic));
...
// 					OnRecvData?.Invoke(this, eBle);                         //上位にイベント通知
// 				}
// 			}
...
// 				}
// 				// var characteristic = await _service.GetCharacteristicAsync(Guid.Parse(_bleconInfo._ReadCharacteristic));
...
// 				var result = await _characteristicread.ReadAsync();
// 				var resultData = result.data;	//データの取得
...
// 					if (IsConnect != ConnectStat.CONNECT)
// 					{
// 						//切断状態だったら無視
...
// 					if (_characteristicwrite.CanWrite == true)
// 					{
// 						int rslt = await _characteristicwrite.WriteAsync(bdata);  // ★ver 2.13から変わった	戻り値がbool -> int(0=成功)
//                         if (rslt != 0)
// 						{
...
// 						_characteristicread.ValueUpdated -= notifyEventHandler;
